...
**Важно:** Если используется `DNSCrypt`, его нужно отключить и прописать обычные DNS в настройках карты, например Google/Cloudflare/Cisco/etc. Сложные цепочки DNS - плохо для `p2p/VPN/mesh`. В результате при подключении будет: **Тип NAT: Port Restricted NAT**.
